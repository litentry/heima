**@heima-network/client-sdk**

***

# @heima-network/client-sdk

## Namespaces

- [request](@heima-network/namespaces/request/README.md)

## Enumerations

- [ConnectionState](enumerations/ConnectionState.md)

## Classes

- [Enclave](classes/Enclave.md)

## Type Aliases

- [DiscordOAuth2Proof](type-aliases/DiscordOAuth2Proof.md)
- [DiscordProof](type-aliases/DiscordProof.md)
- [EmailProof](type-aliases/EmailProof.md)
- [JsonRpcRequest](type-aliases/JsonRpcRequest.md)
- [JsonRpcResponse](type-aliases/JsonRpcResponse.md)
- [Maybe](type-aliases/Maybe.md)
- [OmniAuthData](type-aliases/OmniAuthData.md)
- [SubstrateOrEvmOrBtcAddress](type-aliases/SubstrateOrEvmOrBtcAddress.md)
- [TwitterOAuth2Proof](type-aliases/TwitterOAuth2Proof.md)
- [TwitterProof](type-aliases/TwitterProof.md)
- [Web3Proof](type-aliases/Web3Proof.md)

## Variables

- [enclave](variables/enclave.md)

## Functions

- [createAesOutputType](functions/createAesOutputType.md)
- [createIdentityType](functions/createIdentityType.md)
- [createNativeTaskType](functions/createNativeTaskType.md)
- [createOmniAuth](functions/createOmniAuth.md)
- [createPayloadToSign](functions/createPayloadToSign.md)
- [createRawTaskType](functions/createRawTaskType.md)
- [createValidationDataType](functions/createValidationDataType.md)
- [createVerificationMessage](functions/createVerificationMessage.md)
- [decodeSignature](functions/decodeSignature.md)
- [decrypt](functions/decrypt.md)
- [encrypt](functions/encrypt.md)
- [exportKey](functions/exportKey.md)
- [generate](functions/generate.md)
- [generateNonce12](functions/generateNonce12.md)
- [getAndWaitForAccountStoreCreation](functions/getAndWaitForAccountStoreCreation.md)
- [getSignatureCryptoType](functions/getSignatureCryptoType.md)
- [isWeb3](functions/isWeb3.md)
- [toHash](functions/toHash.md)
- [toPublicKey](functions/toPublicKey.md)
- [u8aToBase64Url](functions/u8aToBase64Url.md)
- [verifySignature](functions/verifySignature.md)
