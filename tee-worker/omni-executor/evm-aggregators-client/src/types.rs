...
impl FromStr for Address {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let address = AlloyAddress::from_str(s)
			.map_err(|_| ClientError::InvalidAddress { address: s.to_string() })?;
		Ok(Self(address))
	}
}
...
impl FromStr for ChainId {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let id = s
			.parse::<u64>()
			.map_err(|_| ClientError::InvalidChainId { chain_id: s.to_string() })?;
		Ok(Self(id))
	}
}
...
impl TokenAmount {
	...
	pub fn from_wei_string(s: &str) -> ClientResult<Self> {
		let decimal = Decimal::from_str(s)
			.map_err(|_| ClientError::InvalidAmount { amount: s.to_string() })?;
		Ok(Self(decimal))
	}
...
impl Slippage {
	pub fn new_bps(bps: u32) -> ClientResult<Self> {
		if bps > 10000 {
			return Err(ClientError::InvalidSlippage { value: bps.to_string() });
		}
		Ok(Self(bps))
	}
...
impl FromStr for Slippage {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let bps = s
			.parse::<u32>()
			.map_err(|_| ClientError::InvalidSlippage { value: s.to_string() })?;
		Self::new_bps(bps)
	}
}
