...
// ================================
lazy_static! {
	pub static ref KYBER_DEX_MAPPINGS: HashMap<&'static str, &'static str> = {
	...
	pub static ref INCH_DEX_MAPPINGS: HashMap<u64, HashMap<String, String>> = {
		let mut map = HashMap::new();
		...
		bsc_map.insert("UniswapV2".to_string(), "BSC_UNISWAP_V2".to_string());
...
lazy_static! {
	pub static ref OKX_DEX_MAPPINGS: HashMap<String, String> = {
		let mut map = HashMap::new();
...
pub fn is_native_token(token: &[u8]) -> bool {
	...
	token_str == WRAPPED_MAINNET_ETH
		...
		|| token_str == NATIVE_TOKEN_ADDRESS
}
