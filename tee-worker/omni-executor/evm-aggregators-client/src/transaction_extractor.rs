...
pub fn extract_transaction_data<T: TransactionDataExtractor>(
	...
	let to = Address::from_hex(extractor.get_to_address()).map_err(|_| {
		ClientError::InvalidAddress { address: extractor.get_to_address().to_string() }
	})?;

	let gas = match extractor.get_gas() {
		TransactionGas::AsU64(gas_val) => Decimal::from_u64(gas_val).ok_or_else(|| {
			ClientError::GasCalculation { reason: "Failed to convert gas to decimal".to_string() }
		})?,
		TransactionGas::AsString(gas_str) => Decimal::from_str(&gas_str).map_err(|_| {
			ClientError::GasCalculation { reason: "Failed to parse gas value".to_string() }
		})?,
	};

	let adjusted_gas =
		gas.mul(Decimal::new(15, 1))
			.to_u64()
			.ok_or_else(|| ClientError::GasCalculation {
				reason: "Failed to convert adjusted gas to u64".to_string(),
			})?;
