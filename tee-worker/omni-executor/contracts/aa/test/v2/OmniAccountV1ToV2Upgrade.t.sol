...
contract OmniAccountV1ToV2Upgrade is Test {
    ...
    function test_ComprehensiveV1ToV2Upgrade() public {
        ...
        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(address(module), abi.encodeWithSignature("pushToArray(uint256)", 111));

        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(address(module), abi.encodeWithSignature("pushToArray(uint256)", 222));

        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(
            address(module), abi.encodeWithSignature("setMapping(address,uint256)", address(0x5555), 9999)
        );
        ...
        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(
            address(module), abi.encodeWithSignature("setStruct(uint256,string,uint256)", 1, "test_struct", 8888)
        );

        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(address(module), abi.encodeWithSignature("setString(string)", "hello_from_module"));

        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(address(module), abi.encodeWithSignature("setBytes(bytes)", hex"deadbeef"));
        ...
        for (uint256 i = 0; i < 5; i++) {
            vm.prank(address(entryPoint));
            accountV2.executeModuleCall(address(module), abi.encodeWithSignature("incrementSimpleValue()"));
        }

        // Push more values to array
        for (uint256 i = 0; i < 3; i++) {
            vm.prank(address(entryPoint));
            accountV2.executeModuleCall(address(module), abi.encodeWithSignature("pushToArray(uint256)", 1000 + i));
        }
        ...
        vm.prank(address(entryPoint));
        accountV2.executeModuleCall(address(module), abi.encodeWithSignature("setSimpleValue(uint256)", 99999));
    }
    ...
    function test_V1ToV2UpgradeNonEvmAccount() public {
        ...
        // Execute module
        vm.prank(address(entryPoint));
        substrateAccountV2.executeModuleCall(address(module), abi.encodeWithSignature("setSimpleValue(uint256)", 777));
