...
import {OmniAccountV2 as OmniAccount} from "../../src/v2/accounts/OmniAccountV2.sol";
import {BaseAccount} from "../../src/v2/core/BaseAccount.sol";
import {EntryPointV1 as EntryPoint} from "../../src/v2/core/EntryPointV1.sol";
...
contract OmniAccountV2AsRootNonEvm is Test {
    ...
    function test_RootCanCallAddRootSignerViaUserOpForNonEvmOwner() public {
        ...
        (counter, entryPoint, account) =
            OmniAccountV2TestUtils.setUpWithOwnerType(ownerAddress, clientId, root, OwnerType.Email);
    ...
    function test_RootCanCallWithdrawDepositViaUserOpForNonEvmOwner() public {
        (address root, uint256 rootPk) = makeAddrAndKey("root");
        (counter, entryPoint, account) =
            OmniAccountV2TestUtils.setUpWithOwnerType(ownerAddress, clientId, root, OwnerType.Google);
    ...
    function test_SessionKeyStillCannotCallRestrictedFunctionsForNonEvmOwner() public {
        ...
        (counter, entryPoint, account) =
            OmniAccountV2TestUtils.setUpWithOwnerType(ownerAddress, clientId, root, OwnerType.Twitter);
    ...
    // ============ Module Management Tests for Non-EVM Accounts ============

    function test_RootCanRegisterModuleDirectlyWhenNoPasskeys() public {
        // Root should be able to register module directly for non-EVM owner with no passkeys
        vm.prank(rootAddress);
    ...
    function test_RootCanUnregisterModuleDirectlyWhenNoPasskeys() public {
        // First register a module
        vm.prank(rootAddress);
        account.registerModule(address(module));
        assertTrue(account.isModuleRegistered(address(module)));

        // Root should be able to unregister module directly
        vm.prank(rootAddress);
    ...
    function test_RootCanRegisterModuleViaUserOpWhenNoPasskeys() public {
        (address root, uint256 rootPk) = makeAddrAndKey("root");
        (counter, entryPoint, account) =
            OmniAccountV2TestUtils.setUpWithOwnerType(ownerAddress, clientId, root, OwnerType.Substrate);
        ...
        // Validation should succeed because root can call restricted functions for non-EVM owners with no passkeys
        vm.prank(address(entryPoint));
        uint256 validationData = account.validateUserOp(packedOp, packedOpHash, 0);
        assertEq(SIG_VALIDATION_SUCCESS, validationData);
        ...
        // Verify the module was actually registered
        assertTrue(account.isModuleRegistered(address(module)));
    }

    function test_RootCannotRegisterModuleDirectlyWhenPasskeyExists() public {
        Passkey.PublicKey memory pk = Passkey.PublicKey({x: 12345, y: 67890});

        // First add a passkey signer (root can do this while they're still owner)
        vm.prank(rootAddress);
        account.addPasskeySigner(pk);
        bytes32 key = Passkey.toKey(pk);
        assertTrue(account.passkeySigners(key));

        // Now root should NOT be able to register module directly
        vm.expectRevert("only owner");
        vm.prank(rootAddress);
        account.registerModule(address(module));
    }

    function test_RootCannotUnregisterModuleDirectlyWhenPasskeyExists() public {
        // First register module while root is owner (no passkeys yet)
        vm.prank(rootAddress);
        account.registerModule(address(module));
        assertTrue(account.isModuleRegistered(address(module)));
        ...
        // Now root should NOT be able to unregister module
        vm.expectRevert("only owner");
        vm.prank(rootAddress);
        account.unregisterModule(address(module));
    }

    function test_RootCannotRegisterModuleViaUserOpWhenPasskeyExists() public {
        (address root, uint256 rootPk) = makeAddrAndKey("root");
        (counter, entryPoint, account) =
            OmniAccountV2TestUtils.setUpWithOwnerType(ownerAddress, clientId, root, OwnerType.Google);

        // Add passkey signer first
        Passkey.PublicKey memory pk = Passkey.PublicKey({x: 12345, y: 67890});
        vm.prank(root);
        account.addPasskeySigner(pk);
        ...
        // Validation should FAIL because passkey signer now exists
        vm.prank(address(entryPoint));
        uint256 validationData = account.validateUserOp(packedOp, packedOpHash, 0);
        assertEq(SIG_VALIDATION_FAILED, validationData);
    }

    function test_RootCannotUnregisterModuleViaUserOpWhenPasskeyExists() public {
        (address root, uint256 rootPk) = makeAddrAndKey("root");
        (counter, entryPoint, account) =
            OmniAccountV2TestUtils.setUpWithOwnerType(ownerAddress, clientId, root, OwnerType.Twitter);

        // Register module first (while root is owner)
        vm.prank(root);
        account.registerModule(address(module));
        assertTrue(account.isModuleRegistered(address(module)));
