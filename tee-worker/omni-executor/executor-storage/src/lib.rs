...
pub trait Storage<K: Encode, V: Codec> {
		...
		match self.db().get(storage_key(self.name(), &key.encode())) {
			Ok(Some(v)) => {
				let decoded_v = V::decode(&mut &v[..]).map_err(|e| {
					tracing::log::error!("Error decoding value from storage: {:?}", e);
				})?;
				Ok(Some(decoded_v))
			},
			Ok(None) => Ok(None),
			Err(e) => {
				tracing::log::error!("Error getting value from storage: {:?}", e);
				Err(())
			},
		}
	}
	...
	fn insert(&self, key: &K, value: V) -> Result<(), ()> {
		...
		self.db()
			...
			.map_err(|e| {
				tracing::log::error!("Error inserting value into storage: {:?}", e);
			})
	}
	...
	fn remove(&self, key: &K) -> Result<(), ()> {
		...
		self.db()
			...
			.map_err(|e| {
				tracing::log::error!("Error removing value from storage: {:?}", e);
			})
	}
}
...
pub async fn init_storage(ws_rpc_endpoint: &str) -> Result<Arc<StorageDB>, ()> {
	let db = Arc::new(StorageDB::open_default(STORAGE_DB_PATH).map_err(|e| {
		tracing::log::error!("Could not open db: {:?}", e);
	})?);
	...
	let mut client = client_factory.new_client().await.map_err(|e| {
		tracing::log::error!("Could not create client: {:?}", e);
	})?;
...
async fn init_omni_account_storages(
	...
	loop {
		let storage_keys_paged = client
			...
			.await
			.map_err(|e| {
				tracing::log::error!("Could not get storage keys paged: {:?}", e);
			})?;
		...
		let proof =
			client
				...
				.await
				.map_err(|e| {
					tracing::log::error!("Could not get storage proof by keys: {:?}", e);
				})?;
		let header = match client.get_last_finalized_header().await {
			Ok(header) => header,
			_ => {
				tracing::log::error!("Could not get last finalized header");
				return Err(());
			},
		};
		...
		.map_err(|e| {
			tracing::log::error!("Could not read proof check: {:?}", e);
		})?;
			...
			match storage_map.get(key) {
				Some(Some(value)) => {
					...
					let maybe_storage_value = client
						...
						.await
						.map_err(|e| {
							tracing::log::error!("Could not get storage at latest block: {:?}", e);
						})?
						.fetch_raw(key.clone())
						.await
						.map_err(|e| {
							tracing::log::error!("Could not fetch storage value: {:?}", e);
						})?;
					let Some(storage_value) = maybe_storage_value else {
						tracing::log::error!(
							"Storage value not found for account_id: {:?}",
							omni_account
						);
						return Err(());
					};
					if storage_value != *value {
						tracing::log::error!(
							"Storage value mismatch for account_id: {:?}",
							omni_account
						);
						return Err(());
					}
					let account_store: AccountStore =
						Decode::decode(&mut &value[..]).map_err(|e| {
							tracing::log::error!("Error decoding account store: {:?}", e);
						})?;
					...
					for member in account_store.0.iter() {
						...
						member_omni_account_storage
							...
							.map_err(|e| {
								tracing::log::error!(
									"Error inserting member account hash: {:?}",
									e
								);
							})?;
					...
					account_store_storage.insert(&omni_account, member_accounts).map_err(|e| {
						tracing::log::error!("Error inserting account store: {:?}", e);
					})?;
				},
				_ => {
					tracing::log::error!("No value found for key: {:?}", key);
				},
			}
		}
	}
...
fn extract_account_id_from_storage_key<K: Decode>(raw_storage_key: &[u8]) -> Result<K, ()> {
	...
	K::decode(&mut raw_key).map_err(|e| {
		tracing::log::error!("Error decoding account id: {:?}", e);
	})
}
