...
#[tracing::instrument(skip(ctx, params), fields(
	...
	from_token_ca = %params.from_token_ca.as_deref().unwrap_or("None"),
	from_amount = %params.from_amount,
	to_chain_id = %params.to_chain_id,
	to_token_ca = %params.to_token_ca.as_deref().unwrap_or("None"),
	wallet_index = %params.wallet_index,
	usd_worth = %params.usd_worth,
	is_cross_chain = %(params.from_chain_id != params.to_chain_id)
))]
async fn handle_submit_swap_order_request<
...
>(
	...
	module
		.register_async_method("pumpx_submitSwapOrder", |params, ctx, _| async move {
			...
			handle_submit_swap_order_request(params, ctx).await
		})
