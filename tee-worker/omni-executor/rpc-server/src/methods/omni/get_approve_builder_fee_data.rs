...
pub struct GetApproveBuilderFeeDataParams {
	pub user_id: UserId,
...
pub struct GetApproveBuilderFeeDataResponse {
	pub main_address: String,
	pub approve_builder_fee_data: ApproveBuilderFeeData,
}

#[derive(Serialize, Clone)]
pub struct ApproveBuilderFeeData {
	pub action: ApproveBuilderFeeAction,
	pub nonce: u64,
...
pub fn register_get_approve_builder_fee_data(module: &mut RpcModule<RpcContext>) {
	module
		.register_async_method("omni_getApproveBuilderFeeData", |params, ctx, _| async move {
			let params = params.parse::<GetApproveBuilderFeeDataParams>().map_err(|e| {
			...
			let action = ApproveBuilderFeeAction {
				...
				builder: Address::from_str(&params.builder).map_err(|_| {
			...
			Ok(GetApproveBuilderFeeDataResponse {
				main_address,
				approve_builder_fee_data: ApproveBuilderFeeData { action, nonce, signature },
			})
		})
		.expect("Failed to register omni_getApproveBuilderFeeData method");
}
...
mod tests {
	...
	#[test]
	fn test_approve_builder_fee_action() {
		let action = ApproveBuilderFeeAction {
	...
	#[test]
	fn test_get_approve_builder_fee_data_params_deserialization() {
		let json = r#"{
		...
		let params: GetApproveBuilderFeeDataParams = serde_json::from_str(json).unwrap();
		...
		assert_eq!(params.max_fee_rate, "0.01");
		assert_eq!(params.builder, "0x742d35Cc6634C0532925a3b844Bc9e7595f02A10");
