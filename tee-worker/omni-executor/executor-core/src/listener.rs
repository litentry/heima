	...
	> Listener<Fetcher, CheckpointT, CheckpointRepositoryT, EventId, BlockEventT, EventHandlerT>
{
	...
	pub fn sync(&mut self, start_block: u64) {
		tracing::log::info!("Starting {} network sync, start block: {}", self.id, start_block);
		...
		tracing::log::debug!("Starting sync from {:?}", block_number_to_sync);
		...
		'main: loop {
			...
			let maybe_last_finalized_block =
				match self.handle.block_on(self.fetcher.get_last_finalized_block_num()) {
					...
					Err(_) => {
						tracing::log::info!("Could not get last finalized block number");
			...
			if last_finalized_block >= block_number_to_sync {
				tracing::log::debug!("Syncing block: {}", block_number_to_sync);
				match self.handle.block_on(self.fetcher.get_block_events(block_number_to_sync)) {
					Ok(events) => {
						for event in events {
							...
							if let Some(ref checkpoint) = maybe_checkpoint {
								if checkpoint.ge(&event.get_event_id().clone().into()) {
									tracing::log::debug!("Skipping event");
									continue;
								}
							}
							tracing::log::debug!("Handling event: {:?}", event_id);
							if let Err(e) = self.handle.block_on(self.event_handler.handle(event)) {
								tracing::log::error!("Could not handle event: {:?}", e);
								match e {
						...
						tracing::log::debug!("Finished syncing block: {}", block_number_to_sync);
						block_number_to_sync += 1;
					},
					Err(e) => {
						tracing::log::error!(
							"Could not get block {} events: {:?}",
							block_number_to_sync,
							e
						);
						sync_error = true;
				...
				tracing::log::trace!("Block: {} not yet finalized", block_number_to_sync);
			}

			if !fast || sync_error {
				sleep(Duration::from_secs(1))
			} else {
				tracing::log::trace!("Fast sync skipping 1s wait");
			}
		}
	}
}
