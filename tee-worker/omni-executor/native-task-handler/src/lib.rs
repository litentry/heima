...
use executor_primitives::{
	utils::hex::ToHexPrefixed, AccountId, Identity, IdentitySerde, Intent, IntentId, MemberAccount,
	OmniAccountAuthType, OmniAuthSerde, ValidationData, Web2IdentityType,
};
...
>(
	...
	let (response_sender, tx) = match wrapper.task {
		...
		NativeTask::UserLogin(user_info) => {
			let expires_at = Utc::now()
				.checked_add_days(Days::new(AUTH_TOKEN_EXPIRATION_DAYS))
				.expect("Failed to calculate expiration")
				.timestamp();
			...
			let (omni_account, access_token) = match user_info.id {
				IdentitySerde::Pumpx(email) => {
					debug!("Calling pumpx get_account_user_id, email: {}", email);
					let Ok(res) = ctx.pumpx_api.get_account_user_id(email.clone()).await else {
						send_error(
							"Failed to get_account_user_id".to_string(),
					...
					let omni_account =
						Identity::from_web2_account(&user_id, Web2IdentityType::Pumpx)
							.to_omni_account();
					...
					let Ok(access_token) =
						jwt::create(&access_token_claims, &ctx.jwt_rsa_private_key)
					else {
						send_error(
							"Failed to create access token".to_string(),
							response_sender,
					...
					let OmniAuthSerde::Pumpx { invite_code, google_code } = user_info.auth else {
						send_error(
							"Invalid auth type for UserLogin".to_string(),
							response_sender,
							NativeTaskError::InternalError,
						);
						return;
					...
					(omni_account, access_token)
				},
				_ => {
					let Ok(identity) = Identity::try_from(user_info.id) else {
						send_error(
							"Invalid identity type for UserLogin".to_string(),
							response_sender,
							NativeTaskError::InternalError,
						);
						return;
					};
					// return access token and id token from here
			...
			todo!("Send response with access_token and id_token");
