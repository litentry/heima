...
/**
 * A library to define Passkey related primitives and signature verification methods
 * Adapted from https://github.com/ithacaxyz/exp-0001/blob/main/contracts/src/utils/WebAuthnP256.sol
 *
 * The P256 verification is from openzeppelin implementatons
 */
...
library Passkey {
    ...
    /// Convert PublicKey to hashable storage key
    function toKey(PublicKey memory pk) internal pure returns (bytes32) {
        return sha256(abi.encodePacked(pk.x, pk.y));
    }
    ...
    /// Verifies the authFlags in authData. Numbers in inline comment
     ...
     * - Verify that authData (which comes from the authenticator,
     ...
     */
    function verify(bytes32 challenge, Metadata memory metadata, Signature memory signature, PublicKey memory pk)
        internal
        view
        returns (bool)
    {
        // Check that authData has good flags
        if (metadata.authData.length < 37 || !checkAuthFlags(metadata.authData[32], metadata.userVerificationRequired))
        {
            return false;
        }
        ...
        string memory challengeB64url = Base64.encodeURL(abi.encodePacked(challenge));
        ...
        // Check that the public key signed sha256(authData || sha256(clientDataJSON))
        bytes32 clientDataJSONHash = sha256(bytes(metadata.clientDataJSON));
        bytes32 messageHash = sha256(abi.encodePacked(metadata.authData, clientDataJSONHash));

        return P256.verify(messageHash, bytes32(signature.r), bytes32(signature.s), bytes32(pk.x), bytes32(pk.y));
    }
}
