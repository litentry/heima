...
export function AAWalletInfo({ onAddressCalculated }: AAWalletInfoProps = {}) {
  ...
  const [clientIdBytes32] = useState<`0x${string}`>(stringToBytes32(DEFAULT_CLIENT_ID))
  // For consistency, we'll always use the user's EVM address as the default root signer
  // This ensures the pre-calculated address matches the deployed address
  const rootSigner = evmAddress || ('0x0000000000000000000000000000000000000000' as Address)

  // Calculate omni account when wallet is connected
  useEffect(() => {
    if (evmAddress) {
      const account = calculateOmniAccount(evmAddress, DEFAULT_CLIENT_ID, 'evm')
  ...
  // Use contract read to get the Smart Account address
      ...
      <div className="w-full p-6 bg-gray-50 rounded-lg border border-gray-200">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Connect your wallet to view your Account Abstraction wallet information.
          </p>
        </div>
      </div>
    )
      ...
      <div className="space-y-6">
        ...
        {evmAddress && (
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-gray-900">Connected EVM Wallet</h3>
            <div className="bg-gray-50 p-3 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Address:</span>
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-mono">{truncateAddress(evmAddress)}</span>
                  <button
        ...
        {solanaAddress && (
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-gray-900">Connected Solana Wallet</h3>
            <div className="bg-gray-50 p-3 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Address:</span>
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-mono">{truncateAddress(solanaAddress.toString())}</span>
                  <button
        ...
        {Boolean(evmAddress && omniAccount && omniAccount.length > 2) && (
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-gray-900">Omni Account</h3>
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-blue-700 font-medium">SHA256 Hash:</span>
                <button
        ...
        {aaWalletAddress && (
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-green-700">Smart Account Address</h3>
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-green-700 font-medium">Counterfactual:</span>
                <div className="flex items-center space-x-2">
                  <button
                  ...
                  <button
                    onClick={() => window.open(`https://etherscan.io/address/${aaWalletAddress}`, '_blank')}
                    className="p-1 hover:bg-green-100 rounded"
                  >
              ...
              <div className="text-xs text-green-600 mt-2">
                This is your Smart Account address - fund this address to start using AA features.
              </div>
            </div>
          </div>
        )}
