...
use oe_client_ethereum::{RpcProvider, RpcProviderError};
...
impl<P: RpcProvider<Transaction = TransactionRequest, Addr = Address>> EntryPointClient<P> {
	...
	pub async fn get_sender_address(&self, init_code: Bytes) -> Result<Address, ()> {
		...
		match self.rpc_client.call(tx).await {
			Err(err) => {
				if let oe_client_ethereum::RpcProviderError::ExecutionReverted {
					reason: _,
					data: Some(data),
				} = &err
				{
	...
	fn classify_rpc_error(
		&self,
		rpc_error: oe_client_ethereum::RpcProviderError,
	) -> AaContractError {
		match rpc_error {
			oe_client_ethereum::RpcProviderError::InvalidUrl(url) => {
				AaContractError::Validation(format!("Invalid RPC URL: {}", url))
			},
			oe_client_ethereum::RpcProviderError::Network(msg) => {
				AaContractError::Rpc(RpcError::ConnectionFailed { endpoint: msg })
			},
			oe_client_ethereum::RpcProviderError::NoWallet => {
				AaContractError::Validation("No wallet configured for signing".to_string())
			},
			oe_client_ethereum::RpcProviderError::ExecutionReverted { reason, .. } => {
				AaContractError::Contract(ContractError::ExecutionReverted { reason })
			},
			oe_client_ethereum::RpcProviderError::JsonRpc { code, message, data } => {
			...
			oe_client_ethereum::RpcProviderError::Transaction(msg) => {
			...
			oe_client_ethereum::RpcProviderError::Generic(msg) => AaContractError::Generic(msg),
		}
	}
}
...
pub mod test {
	...
	pub async fn test_get_sender_address() {
		...
		rpc_client
			...
			.returning(|_| {
				...
				Err(oe_client_ethereum::RpcProviderError::ExecutionReverted { reason, data: Some(hex::decode("0x6ca7b8060000000000000000000000005dfec187c82986cf670f4e2ed6de1cd001cee5be").unwrap()) })
			});
	...
	pub async fn test_dynamic_gas_pricing() {
		...
		rpc_client.expect_estimate_eip1559_fees().times(1).returning(|| {
			Err(oe_client_ethereum::RpcProviderError::Generic("EIP-1559 not supported".to_string()))
		});
	...
	async fn test_handle_ops_with_retry_success() {
		...
		mock_client.expect_send_transaction().times(3).returning(move |_| {
			...
			if count < 2 {
				Err(oe_client_ethereum::RpcProviderError::Network(
	...
	async fn test_handle_ops_with_retry_max_attempts() {
		...
		mock_client.expect_send_transaction().times(3).returning(|_| {
			Err(oe_client_ethereum::RpcProviderError::Network("Connection failed".to_string()))
		});
	...
	async fn test_calculate_gas_fees_with_eip1559_support() {
		use oe_client_ethereum::Eip1559FeeEstimate;
		let mut mock_client = MockRpcProvider::new();
	...
	fn test_error_classification() {
		use crate::error::{AaContractError, ContractError, RpcError};
		use oe_client_ethereum::RpcProviderError;
	...
	async fn test_handle_ops_with_retry_non_retryable() {
		...
		mock_client.expect_send_transaction().times(1).returning(|_| {
			Err(oe_client_ethereum::RpcProviderError::ExecutionReverted {
