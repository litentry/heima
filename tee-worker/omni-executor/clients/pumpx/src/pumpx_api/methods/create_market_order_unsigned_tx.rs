...
pub async fn create_market_order_unsigned_tx_impl(
	...
	let response = client
		...
		.await
		.map_err(|e| {
			error!("Failed to send create_market_order_unsigned_tx request: {:?}", e);
			e
		})?;

	let status = response.status();
	let response = response.error_for_status().map_err(|e| {
		error!("create_market_order_unsigned_tx failed with status: {}, error: {:?}", status, e);
		e
	})?;

	response.json().await.map_err(|e| {
		error!("Failed to parse create_market_order_unsigned_tx response: {:?}", e);
		e
	})
}
