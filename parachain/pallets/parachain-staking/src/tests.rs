...
fn set_blocks_per_round_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(ParachainStaking::set_blocks_per_round(RuntimeOrigin::root(), 12u32));
...
fn cannot_join_candidates_if_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				ParachainStaking::join_candidates(RuntimeOrigin::signed(1), 11u128),
...
fn cannot_join_candidates_if_delegator() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(RuntimeOrigin::root(), 2));
			assert_noop!(
				ParachainStaking::join_candidates(RuntimeOrigin::signed(2), 10u128),
...
fn cannot_join_candidates_with_more_than_available_balance() {
	ExtBuilder::default().with_balances(vec![(1, 500)]).build().execute_with(|| {
		...
		assert_ok!(ParachainStaking::add_candidates_whitelist(RuntimeOrigin::root(), 1));
		assert_noop!(
			ParachainStaking::join_candidates(RuntimeOrigin::signed(1), 501u128),
...
fn insufficient_join_candidates_weight_hint_fails() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(RuntimeOrigin::root(), 6));
		});
}
...
fn sufficient_join_candidates_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 6..10 {
				// This should be safe to delete after collator restrcition removed
				assert_ok!(ParachainStaking::add_candidates_whitelist(RuntimeOrigin::root(), i));
				assert_ok!(ParachainStaking::join_candidates(RuntimeOrigin::signed(i), 20));
			}
		});
}
...
fn cannot_leave_candidates_if_not_candidate() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(1)),
...
fn sufficient_leave_candidates_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 1..6 {
				assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(i)));
			}
		});
}
...
fn execute_leave_candidates_callable_by_any_signed() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(1)));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_candidates(RuntimeOrigin::signed(2), 1));
		});
}
...
fn cannot_execute_leave_candidates_before_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(1)));
			assert_noop!(
				ParachainStaking::execute_leave_candidates(RuntimeOrigin::signed(3), 1),
			...
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_candidates(RuntimeOrigin::signed(3), 1));
		});
}
...
fn cancel_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(1)));
			assert_ok!(ParachainStaking::cancel_leave_candidates(RuntimeOrigin::signed(1)));
...
fn can_delegate_immediately_after_other_join_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(RuntimeOrigin::root(), 1));
			assert_ok!(ParachainStaking::join_candidates(RuntimeOrigin::signed(1), 20));
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(2), 1, 20));
		});
}
...
fn can_delegate_if_revoking() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(2), 4, 10));
		});
}
...
fn cannot_delegate_if_full_and_new_delegation_less_than_or_equal_lowest_bottom() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				ParachainStaking::delegate(RuntimeOrigin::signed(11), 1, 10),
...
fn can_delegate_if_full_and_new_delegation_greater_than_lowest_bottom() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(11), 1, 11));
...
fn can_still_delegate_if_leaving() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(2), 3, 10),);
		});
}
...
fn cannot_delegate_if_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				ParachainStaking::delegate(RuntimeOrigin::signed(2), 1, 10),
				Error::<Test>::CandidateExists
			);
		});
}
...
fn cannot_delegate_more_than_max_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				ParachainStaking::delegate(RuntimeOrigin::signed(2), 6, 10),
...
fn sufficient_delegate_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			for i in 7..11 {
				assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(i), 1, 10));
			}
			for i in 3..11 {
				assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(i), 2, 10));
			}
		});
}
...
fn execute_leave_delegators_removes_delegations_from_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_delegators(RuntimeOrigin::signed(1), 1));
			for i in 2..6 {
				let candidate_state =
					ParachainStaking::candidate_info(i).expect("initialized in ext builder");
...
fn sufficient_execute_leave_delegators_weight_hint_succeeds() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			for i in 3..7 {
				assert_ok!(ParachainStaking::execute_leave_delegators(RuntimeOrigin::signed(i), i));
			}
		});
}
...
fn delegator_schedule_revocation_total() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(2), 5, 10));
			assert_ok!(ParachainStaking::schedule_revoke_delegation(RuntimeOrigin::signed(2), 3));
...
fn parachain_bond_inflation_reserve_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(25);
			assert_ok!(ParachainStaking::execute_leave_delegators(RuntimeOrigin::signed(6), 6));
			roll_to(30);
			let mut new2 = vec![
			...
			set_author(8, 1, 100);
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(8), 1, 10));
			roll_to(45);
...
fn paid_collator_commission_matches_config() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_events!(expected.clone());
			// This should be safe to delete after collator restrcition removed
			assert_ok!(ParachainStaking::add_candidates_whitelist(RuntimeOrigin::root(), 4));
			assert_ok!(ParachainStaking::join_candidates(RuntimeOrigin::signed(4), 20u128));
			assert_last_event!(MetaEvent::ParachainStaking(Event::JoinedCollatorCandidates {
				account: 4,
			...
			roll_to(9);
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(5), 4, 10));
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(6), 4, 10));
			roll_to(11);
			let mut new = vec![
...
fn collator_exit_executes_after_delay() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			roll_to(11);
			assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(2)));
			let info = ParachainStaking::candidate_info(&2).unwrap();
			assert_eq!(info.status, CollatorStatus::Leaving(5));
			roll_to(21);
			assert_ok!(ParachainStaking::execute_leave_candidates(RuntimeOrigin::signed(2), 2));
...
fn collator_selection_chooses_top_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_events!(expected);
			assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(6)));
			assert_last_event!(MetaEvent::ParachainStaking(Event::CandidateScheduledExit {
			...
			roll_to(21);
			assert_ok!(ParachainStaking::execute_leave_candidates(RuntimeOrigin::signed(6), 6));
			assert_ok!(ParachainStaking::join_candidates(RuntimeOrigin::signed(6), 69u128));
			assert_last_event!(MetaEvent::ParachainStaking(Event::JoinedCollatorCandidates {
				account: 6,
...
fn multiple_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_events!(expected.clone());
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(6), 2, 10));
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(6), 3, 10));
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(6), 4, 10));
			roll_to(16);
			let mut new = vec![
				Event::Delegation {
			...
			assert_eq_events!(expected.clone());
			roll_to(21);
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(7), 2, 80));
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(10), 2, 10));
			roll_to(26);
			let mut new2 = vec![
				Event::CollatorChosen { round: 5, collator_account: 1, total_exposed_amount: 50 },
...
fn delegation_events_convey_correct_position() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(7), 1, 15));
...
fn delegation_kicked_from_bottom_removes_pending_request() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(10), 1, 20));
			// check the event
			assert_event_emitted!(Event::DelegationKicked {
				delegator: 2,
				candidate: 1,
...
fn no_selected_candidates_defaults_to_last_round_collators() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// execute leave
			for i in 1..6 {
				assert_ok!(ParachainStaking::execute_leave_candidates(RuntimeOrigin::signed(i), i));
			}
			// next round
...
fn remove_white_list() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::remove_candidates_whitelist(RuntimeOrigin::root(), 1));
			...
			assert_ok!(ParachainStaking::remove_candidates_whitelist(RuntimeOrigin::root(), 3));
			assert_last_event!(MetaEvent::ParachainStaking(Event::CandidateWhiteListRemoved {
				candidate: 1,
			}));
		});
...
// Test of reserve, lock, transfer availability in staking
#[test]
fn staked_capacity() {
	ExtBuilder::default()
		.with_balances(vec![(1, 1000), (2, 1000), (3, 1000), (4, 1000), (5, 1000), (6, 1000)])
		.with_candidates(vec![(1, 100), (2, 100), (3, 100)])
		.with_delegations(vec![])
		.build()
		.execute_with(|| {
			const ID_1: LockIdentifier = *b"1       ";
			// Delegate/Candidate can not stake more if they do not have enough transferrable (free)
			// balance
			assert_noop!(
				ParachainStaking::candidate_bond_more(RuntimeOrigin::signed(1), 3000),
				DispatchError::Module(ModuleError {
					index: 2,
					error: [8, 0, 0, 0],
					message: Some("InsufficientBalance")
			...
			assert_noop!(
				ParachainStaking::delegate(RuntimeOrigin::signed(4), 1, 3000),
				DispatchError::Module(ModuleError {
					index: 2,
					error: [8, 0, 0, 0],
					message: Some("InsufficientBalance")
			...
			// Delegate/Candidate can stake more if they enough balance and lock insignificant large
			...
			Balances::set_lock(ID_1, &5, u128::MAX, WithdrawReasons::all());
			// Delegator leaves freely
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(
			...
			assert_ok!(ParachainStaking::schedule_leave_delegators(RuntimeOrigin::signed(5)));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_delegators(RuntimeOrigin::signed(5), 5));
			assert!(ParachainStaking::delegator_state(2).is_none());
			assert!(
				!ParachainStaking::delegation_scheduled_requests(&1)
					.iter()
			...
			// Candidate leaves freely
			assert_ok!(ParachainStaking::schedule_candidate_bond_less(RuntimeOrigin::signed(2), 5));
			roll_to(20);
			assert_ok!(ParachainStaking::execute_candidate_bond_less(RuntimeOrigin::signed(2), 2));
			assert_last_event!(MetaEvent::ParachainStaking(Event::CandidateBondedLess {
				candidate: 2,
				amount: 5,
				new_bond: 595,
