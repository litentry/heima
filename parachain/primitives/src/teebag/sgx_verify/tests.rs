...
fn deserialize_qe_identity_works() {
	...
	let leaf_cert = Certificate::from_der(&certs[0]).unwrap();
	verify_cert_chain(&certs, COLLATERAL_VERIFICATION_TIMESTAMP).unwrap();
...
fn verify_tcb_info_signature() {
	...
	let cert_bytes = BASE64_STANDARD.decode(cert).unwrap();
	...
	extract_cert_pubkey(&cert)
		.and_then(|k| verify_raw_sig(k, data, &signature))
		.unwrap();
}
...
fn parse_pck_certificate() {
	...
	let der = BASE64_STANDARD.decode(der).unwrap();
